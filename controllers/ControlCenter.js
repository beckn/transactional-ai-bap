...
export const networkSwitch = async (req, res) => {
    try {
        const filePath = path.join(appRootPath.toString(), `/tmp/status.json`);
        const status = JSON.parse(fs.readFileSync(filePath, "utf8"));
        status.network_on = !status.network_on;
        fs.writeFileSync(filePath, JSON.stringify(status));
        return res.status(200).json({
            message: status.network_on ? 'Agent is now connected to network' : 'Agent is now disconnected from network',
            status: true
        })
    } catch (error) {
        logger.error(error.message)
