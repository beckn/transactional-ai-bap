...
const ai = new AI()
const on_search = JSON.parse(
    readFileSync('./tests/data/api_responses/on_search.json')
)
const on_search_compressed = JSON.parse(
    readFileSync('./tests/data/api_responses/on_search_compressed.json')
)
const on_select = JSON.parse(
    readFileSync('./tests/data/api_responses/on_select.json')
)
const on_init = JSON.parse(
    readFileSync('./tests/data/api_responses/on_init.json')
)
...
describe('Test cases for services/ai/get_beckn_action_from_text()', () => {
    it('Should test get_beckn_action_from_text() succesfully for a search intent', async () => {
        const response = await ai.get_beckn_action_from_text(
            "I'm looking for some ev chargers."
        )
    ...
    it('Should test get_beckn_action_from_text() succesfully for a select intent', async () => {
        const response = await ai.get_beckn_action_from_text(
            'I want to add this to the cart.'
        )
    ...
    it('Should test get_beckn_action_from_text() succesfully for an itinerary', async () => {
        const response = await ai.get_beckn_action_from_text(
            'I want to plan a trip to the mountains. Can you please help me with that?'
        )
...
describe('Test cases for services/ai/get_beckn_request_from_text()', () => {
    it('Should test get_beckn_request_from_text() succesfully for a `search` intent', async () => {
        const response = await ai.get_beckn_request_from_text(
            "I'm looking for some ev chargers."
        )
        ...
        expect(response.data.body.message.intent.descriptor).to.have.property(
            'name'
        )
    })
    it('Should test get_beckn_request_from_text() succesfully for a `select`', async () => {
        let context = [
            ...
            {
                role: 'assistant',
                content: JSON.stringify(on_search_compressed),
            },
        ]
        const response = await ai.get_beckn_request_from_text(
            'Lets select the first item',
            context
        )
        expect(response.data).to.be.an('object')
        expect(response.data.method.toUpperCase()).to.be.eq('POST')
    ...
    it('Should test get_beckn_request_from_text() succesfully for a `init`', async () => {
        ...
        const response = await ai.get_beckn_request_from_text(
            'Lets place the order. My details are : Mayur Virendra, 9986949245, mayurlibra@gmail.com',
            context
        )
        expect(response.data).to.be.an('object')
        expect(response.data.method.toUpperCase()).to.be.eq('POST')
        ...
        expect(response.data.body.message.order.billing).to.have.property(
            'name'
        )
        expect(response.data.body.message.order.billing).to.have.property(
            'email'
